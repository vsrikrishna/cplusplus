...
int main(){
    int elements,num;
    long long int product = 1; //This was main trick. Unless long long int is used all test cases was not passing.
	int zeroCount = 0;

    cin >> elements;	
    
    int *inputNum = new int[elements];
    ...
    for(int i=0; i < elements; i++){
	    if (zeroCount == 0) {
            cout<< product / inputNum[i] << endl;
        } 
		else if ((zeroCount == 1 && inputNum[i] != 0) ||(zeroCount > 1)) {
            cout << 0 << endl;
        } 
		else if (zeroCount == 1 && inputNum[i] == 0) {
            cout<< product << endl;
        }
	}
