...
void search(string pat, string txt, int* suffArr, int n)
{
	...
	while (l <= r)
	{
		...
		cout << "suffix arr of mid is " << suffArr[mid] << endl;
		string subtxt = txt.substr(suffArr[mid], m);
		int res = pat.compare(subtxt);
...
int main()
{
	...
	char pat[] = "banana";   // pattern to be searched in text

	// Build suffix array
	int n = strlen(txt);
	int *suffArr = buildSuffixArray(txt, n);

	cout << "The following is suffix array:" << endl;
	printArr(suffArr, n);
